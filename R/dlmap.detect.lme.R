...
function(input, filestem)
{
  ...
  test <- dlmap.test.lme(input, chrSet=c(1:input$n.chr))
  ...
  while (found==1) 
  {
	...
	# Count the number of QTLs which have already been mapped on the selected chromosome
     	...
     	map.results <- dlmap.map.det.lme(input, chrSet[jj], chrSet, loc.temp)
	...
	# Test previous subset of chromosomes for significance 
  	test <- dlmap.test.lme(input, chrSet, prevLoc=locations)
