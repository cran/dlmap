...
function(input, chrSet, prevLoc=NULL, ...)
{
  ...
  for (ii in 1:n.chr)
  if (length(prevLoc[[paste("m_", ii, "chr", sep="")]])!=0)
  ...
  if (length(f.mrk)>0)
  formula$fixed <- paste(formula$fixed, "+", paste(names(dfMerged)[f.mrk], collapse="+"), sep="")
  ...
  for (ii in 1:n.chr)
	formula$group[[paste("g_", ii, "chr", sep="")]] <- setdiff((start.i+sum(n.pos)+c((cummrk[ii]+1):cummrk[ii+1])), f.mrk)
  ...
  {
    dfMrk <- input$dfMrk
    namesrnd <- names(dfMrk)[setdiff(2:ncol(dfMrk), f.mrk-sum(n.pos)+1)]
