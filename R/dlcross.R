`dlcross` <- function(format=c("csv", "csvr", "csvs", "csvsr", "mm", "qtx", "qtlcart", "gary", "karl", "rqtl", "dlmap", "other"), genobj, pheobj, mapobj, idname="ID", dir="", file, genfile, mapfile, phefile, chridfile, mnamesfile, pnamesfile, na.strings=c("-", "NA"), genotypes=c("A", "H", "B", "D", "C"), alleles=c("A", "B"), type, step=0, fixpos=0, estmap=TRUE, ...)
{
  results <- list()
  ...
  if (exists("inCross")) type <- class(inCross)[1]
  ...
  # is it possible to create another function to do most of this just to simplify code?
  if (format=="dlmap" | format=="other")
  {
   ...
   if (missing(type)) 
	stop("Use 'other' format or input 'type' argument")
  ...
  if (any(mapobj[,1]!=colnames(genobj)[2:ncol(genobj)]))
  {
	...
	genobj[,2:ncol(genobj)] <- genobj[,match(mapobj[,1], colnames(genobj)[2:ncol(genobj)])]
	colnames(genobj)[2:ncol(genobj)] <- mapobj[,1]
  }
  ...
  class(map) <- "map"
	...
	inCross$geno <- list()
  	...
  	for (i in 1:length(map))
  	{
  	  inCross$geno[[i]] <- list()
	  if (chrnames[i]=="X")	
	    class(inCross$geno) <- "X" else class(inCross$geno) <- "A"
	  inCross$geno[[i]]$data <- genobj[,match(names(map[[i]]), colnames(genobj))]
